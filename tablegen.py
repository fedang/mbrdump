...
system_ids = [[] for i in range(256)]

for line in IDS.strip().splitlines():
    byte, name = line.split(maxsplit=1)
    system_ids[int(byte, 16)].append(name)

f = open("table.c", "w")
f.write(f"// Source: {SOURCE}\n\n")
f.write("const char *system_ids[][256] = {\n")

for byte, names in enumerate(system_ids):
